                        ...
                        <div class="ibox-content">
                            <div class="table-responsive">
                                ...
                                <table id="table_id_example" class="table table-striped table-bordered text-center">
                                    ...
                                    <tbody>
                                    <?php foreach($playmethod as $vo):?>
                                        <tr id="<?=$vo['id']?>">
                                            <td><?=$vo->lottery->name?></td>
                                            <td><?=$vo->name?></td>
                                            <td><?=$vo->nam->name?></td>
                                            <td id="st"><?php
                                                if($vo['status'] == 1){
                                                ...
                                                ?></td>
                                            <td><?=$vo->note?></td>
                                            <td><a class="btn btn-info btn-xs" href="<?=Url::toRoute(['update','id'=>$vo['id']])?>"><i class="fa fa-edit"></i>修改</a>&emsp;<a class="btn btn-info btn-xs" id="normal">正常</a>&emsp;<a class="btn btn-warning btn-xs" id="normal1">停售</a>&emsp;<a class="btn btn-danger btn-xs" id="normal2">维护</a></td>
                                        </tr>
                                    <?php endforeach;?>
                                    </tbody>
                                </table>
...
$url = \yii\helpers\Url::to(['play-method/status']);
$js =
    <<<JS
