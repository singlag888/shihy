                        ...
                        <div class="ibox-content">
                            <div class="table-responsive">
                                <table id="table_id_example" class="table table-striped table-bordered text-center">
                                    ...
                                    <tbody>
                                    <?php foreach($genre as $vo):?>
                                        <tr id="<?=$vo['id']?>">
                                            <td><?=$vo['id']?></td>
                                            <td><?=$vo['type']==1?'用户类型':'层级'?></td>
                                            <td><?=$vo['name']?></td>
                                            <td><?=$vo['status']==1?'显示':'隐藏'?></td>
                                            <td><a class="btn btn-primary btn-xs" href="<?=Url::toRoute(['genre/update','id'=>$vo['id']])?>"><i class="fa fa-edit"></i>编辑</a>  <a id="del" class="btn btn-default btn-xs">删除</a></td>
                                        </tr>
                                    <?php endforeach;?>
                                    </tbody>
                                </table>
...
$url = \yii\helpers\Url::to(['genre/delete']);
$js =
    <<<JS
